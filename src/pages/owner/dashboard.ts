...
export async function renderOwnerDashboard() {
  ...
  content.innerHTML = `
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      ${metricCard("Biens", String(total))}
      ${metricCard("Occupés", `${occupied} / ${total}`)}
      ${metricCard("Taux d'occupation", `${occupancyRate}%`)}
      ${metricCard("Revenus ce mois-ci", `${monthlyRevenue.toLocaleString("fr-FR")} ${currency}`)}
    </div>
  `;
}
